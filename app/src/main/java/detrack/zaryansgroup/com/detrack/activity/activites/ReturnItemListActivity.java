...
public class ReturnItemListActivity extends AppCompatActivity {
    ...
    private void InitilizeAlaram() {
        Intent intent = new Intent(ReturnItemListActivity.this, GPSService.class);
        pintent = PendingIntent.getService(ReturnItemListActivity.this, 0, intent, 0);
            ...
            public void onClick(DialogInterface dialog, int item) {
                ...
                if (item == 4) {
                    ...
                    if (updatedQty < model.getDelivered_Quantity()) {
                        subtractBill(subtractqty + "", model.getRetailPiecePrice() + "");
                    }
                    if (updatedQty > model.getDelivered_Quantity()) {
                        addtBill(addqty + "", model.getRetailPiecePrice() + "");
                    }
                    model.setRejectReason(reject_reason_list[item].toString());
...
//                    onResume();
    ...
    private void updateReturndBill(int position, int newQty) {
        ...
        float ctnWSPrice = list.get(position).getWSCtnPrice();
        ...
        float PiecesRetailPrice = list.get(position).getRetailPiecePrice();
    ...
    private void parseJsonFeedItems(JSONArray feedArray) {
        try {
            ...
            for (int i = 0; i < feedArray.length(); i++) {
                ...
                model.setWSCtnPrice(feedObj.getLong("WSCtnPrice"));
                ...
                model.setRetailPiecePrice(feedObj.getLong("RetailPiecePrice"));
