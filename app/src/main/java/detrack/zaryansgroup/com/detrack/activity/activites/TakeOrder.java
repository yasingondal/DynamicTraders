...
//                                db.insertRunTimeOrderDetails(filteredItemList, RCusotmerModel);
    ...
    private void InitilizeAlaram() {
        Intent intent = new Intent(TakeOrder.this, GPSService.class);
        pintent = PendingIntent.getService(TakeOrder.this, 0, intent, 0);
...
//        }
    ...
    private void SelectQuantityDiloge(final int position, final AdapterView adapterView) {
        ...
        etPiecePrice.setText(String.valueOf((int) list.get(position).getRetailPiecePrice()));
    ...
    public void findTotal() {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i).isSelected()) {
                gross = gross + (list.get(i).getPcs_qty() * list.get(i).getRetailPiecePrice());
