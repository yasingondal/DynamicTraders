...
public class TotalBillActivity extends AppCompatActivity {
    ...
    TextView discountPercentage, grossTV, discountTotalTV, NetTotalTV, dicountPerTV, orderNetTotal,tvItemGst,lbl_discount1,lbl_discount;
    ImageButton btnMenu;
    ...
    private void HideDiscountForSalesPerson() {
       ...
       String designation = new SharedPrefs(TotalBillActivity.this).getDesignation();
...
//                    Log.d("bank_detailsnames","=>"+RouteNameList1.get(i));
    ...
    private void InitilizeAlaram() {
        Intent intent = new Intent(TotalBillActivity.this, GPSService.class);
        pintent = PendingIntent.getService(TotalBillActivity.this, 0, intent, 0);
    ...
    private void initialization()
    {
        ...
        Log.d("FOCvalue","="+selectedItemList.get(0).getFoc_qty());
        RCusotmerModel = (RegisterdCustomerModel) getIntent().getSerializableExtra("RCCustomer");
        Log.d("selectedItemList", String.valueOf(selectedItemList.size()));
...
//        actionbar = v.findViewById(R.id.actionBarTextView);
    ...
    private void findTotal() {
        float gross = 0, discount = 0, gst = 0, net = 0, rSpecialCustomerPrice=0;
        ...
        for (int i = 0; i < selectedItemList.size(); i++) {
            ...
            selectedItemList.get(i).getTaxCode().equalsIgnoreCase("SR")){
                ...
                {
                    gross = gross + (selectedItemList.get(i).getWSCtnPrice() * selectedItemList.get(i).getCtn_qty());
                    net = net + (selectedItemList.get(i).getWSCtnPrice() * selectedItemList.get(i).getCtn_qty());
                }
                else{
        ...
        if(sharedPrefs.getWholeSalePriceTax().equalsIgnoreCase("Including Tax")){
            float TempSum = (float) Double.parseDouble(new DecimalFormat("##.##").format(net-gst));
            NetTotalTV.setText(String.valueOf(Math.abs(TempSum)));
            //NetTotalTV.setText(String.valueOf((net-gst)));
            float TempNetTotal = net - dis;
            orderNetTotal.setText(String.valueOf(Math.abs(TempNetTotal)));
            //orderNetTotal.setText(String.valueOf(net - dis));
            ...
            NetTotalTV.setText(String.valueOf((Math.abs(net))));
            //orderNetTotal.setText(String.valueOf(net + gst));
            float tempnetTotal = net + gst;
            orderNetTotal.setText(String.valueOf(Math.abs(tempnetTotal)));
    ...
    private void placeOrderDilog() {
        ...
        Log.d("lastrowid",String.valueOf(lastrowId));
...
//
                            ...
                            if (SelectProductActivity.addOrder) {
                                ...
                                if(deliveryInfo != null) RCusotmerModel.setServerOrderId(deliveryInfo.getServer_Delivery_id());
    ...
    private void rInsertVisitRecordInLocalDb(RegisterdCustomerModel rCusotmerModel) {
        ...
        model.setLatitude(rCusotmerModel.getLat());
        model.setLongitude(rCusotmerModel.getLng());
        ...
        model.setStatusID(3);
        model.setIsSync(0);
        model.setVisitStatus("Delivered");
